...
```{r CleanData, results="hide", warning=FALSE, message=FALSE}
...
long_rides <- long_rides %>%
  group_by(member_casual) %>%
  summarize(n = n(),
            mean_length = mean(Long_Ride_Length))
...
* Reference used for "Time of Day" (tod_char): 
  ...
  + Evening = [18:00-21:59]
  + Night = [22:00-04:59]
* There are 3,800 trips of over 24 hours still included in the dataset (mean_length = mean() of the length of all trips over 1440 minutes (24 hours), in minutes):
`r long_rides`
...
```{r EA, warning=FALSE, message=FALSE, tidy=TRUE}

#How do casual customers and members differ in:
